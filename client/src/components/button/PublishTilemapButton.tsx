...
export default function PublishTilemapButton(props: {
  id: string;
  name: string;
}) {
  ...
  const addTag = () => {
    if (tags.length >= TAG_LIMIT) {
      snack.showErrorMessage(
        `You've reached the maximum amount of tags you can have!`
      );
      return;
    }
    if (tag) {
      if (tag.length > TAG_CHAR_LIMIT) {
        snack.showErrorMessage(
          `You're tag can't be more than ${TAG_CHAR_LIMIT} characters!`
        );
        return;
      }
  ...
  const modal = (
    ...
    >
      ...
      <DialogActions>
        <Button disabled={dis} onClick={publish} startIcon={<Send />}>
          Publish
        </Button>
