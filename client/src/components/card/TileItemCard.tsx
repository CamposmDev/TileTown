...
const TileItemCard = (props: Props) => {
    ...
    const headerBox = 
        <Tooltip title={props.tileset.name} followCursor>
            <div className='title-header'>
                <Typography noWrap variant='body2'>{props.tileset.name}</Typography>
                <div >
                <Typography noWrap variant='caption'>By:&ensp;{props.tileset.owner}</Typography>
    ...
    return (
        <Grid item>
            <Card onClick={() => setOpen(true)} sx={{boxShadow: 3}}>
                <CardActionArea >
                    <ImageListItem>
                        <img id={'tile-preview'} src={props.tileset.image} alt=''
                        />
                    {headerBox}
                    {/* <SocialBox 
                        views={props.views} 
