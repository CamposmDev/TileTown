...
export class TilesetEditStore {
  ...
  // TODO: the blob should not be optional
  public async saveTileset(snack?: SnackStore): Promise<void> {
    const f = new FormData();

    f.append("image", this._state.image);
    f.append("tileset", JSON.stringify(this._state.tilesetChanges));

    TilesetApi.updateTilesetById(this._state.tileset.id, f)
      .then((res) => {
        console.log(res);
        this.handleAction({
  ...
  public async saveImage(imageData: string, blob: Blob): Promise<void> {
    this.handleAction({
      ...
      payload: { imageData: imageData, image: blob },
    });
  }
  ...
  protected handleAction(action: TilesetEditorAction): void {
    ...
    switch (type) {
      ...
      case TilesetEditorActionType.SAVE_IMAGE_DATA: {
        this.handleSaveImage(payload.imageData, payload.image);
        break;
      }
  ...
  protected handleSaveImage(imageData: string, image: Blob) {
    this.setEdit({
