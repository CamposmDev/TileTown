...
export default class TilesetController {
    ...
    public async createTileset(req: Request, res: Response): Promise<Response> {
        ...
        let updatedUser = await db.users.updateUser(req.userId, {tilesets: user.tilesets});
        if (updatedUser === null) {
            return res.status(500).json({ message: "Server Error. Error while updating tilesets"});
        }
