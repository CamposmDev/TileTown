...
export default class MongooseTilesetDBM implements TilesetDBM {
    ...
    async createTileset(userId: string, tileset: Partial<Tileset>): Promise<Tileset | null> {
        ...
        let newTileset = new TilesetModel({
            columns: tileset.columns ? tileset.columns : 0,
            image: tileset.image ? tileset.image : "dummy.jpg",
            imageHeight: tileset.imageHeight ? tileset.imageHeight : 0,
            imageWidth: tileset.imageWidth ? tileset.imageWidth : 0,
    ...
    async updateTilesetById(tilesetId: string, partial: Partial<Tileset>): Promise<Tileset | null> {
        if (!mongoose.Types.ObjectId.isValid(tilesetId)) { return null; }
    ...
    protected fillTileset(tileset: TilesetSchemaType & {_id: mongoose.Types.ObjectId}, partial: Partial<Tileset>): void {
        tileset.columns = partial.columns ? partial.columns : tileset.columns;
        tileset.image = partial.image ? partial.image : tileset.image;
        tileset.imageHeight = partial.imageHeight ? partial.imageHeight : tileset.imageHeight;
        tileset.imageWidth = partial.imageWidth ? partial.imageWidth : tileset.imageWidth;
        tileset.margin = partial.margin ? partial.margin : tileset.margin;
        tileset.name = partial.name ? partial.name : tileset.name;
        tileset.properties = partial.properties ? <PropertySchemaType[]>partial.properties : tileset.properties;
        tileset.isPublished = partial.isPublished ? partial.isPublished : tileset.isPublished;
        tileset.updatedAt = partial.lastSaveDate ? partial.lastSaveDate : tileset.updatedAt;
    }
